...
**중요:** 시트 이름은 자동으로 감지됩니다. 한국어 "시트1", 영어 "Sheet1" 등 어떤 이름이든 상관없이 첫 번째 시트를 자동으로 사용합니다.
...
**오류: "Unable to parse range: Sheet1!A:M"** (해결됨 ✅)
- 이 오류는 더 이상 발생하지 않습니다
- 시스템이 자동으로 첫 번째 시트 이름을 감지합니다
- 한국어 "시트1"이든 영어 "Sheet1"이든 상관없이 작동합니다

**오류: "카드 추가 실패"**
- 구글 시트가 Service Account와 공유되어 있는지 확인 (편집자 권한)
- 구글 시트 URL이 올바른지 확인
- 첫 번째 시트에 13개 컬럼 헤더가 올바르게 설정되어 있는지 확인
